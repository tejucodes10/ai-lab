...
class Node:
    def __init__(self, id, parent, cost, heuristic, remaining_cost):
        self.id = id
        self.parent = parent
        self.cost = cost
    ...
    def total_cost(self):
        return self.cost + self.heuristic + self.remaining_cost

def ao_star(start_node, end_node):
    open_list = [Node(start_node, None, 0, heuristic_estimate(start_node, end_node), float('inf'))]
    closed_list = set()
    ...
    while open_list:
        ...
        root.update()
        root.after(1000)
        ao_star_path.append(current_node.id)
        ...
        for neighbor in g.graph[current_node.id]:
            if neighbor not in closed_list:
                ...
                remaining_cost = heuristic_estimate(neighbor, end_node)  # For AO*
                new_node = Node(neighbor, current_node.id, cost, heuristic, remaining_cost)
                open_list.append(new_node)

    print("AO* Path:", ao_star_path)
...
start_button = tk.Button(root, text="Start AO*", command=start_ao_star)
start_button.pack()
